...
This document covers the hardware, the software, every command, every error we hit (with the real error
...
## 12. Visualization tools (ours) & pros/cons

**Our analysis figures (this repo):**
- `testrun/june7_set7/set7_dedrift_compare.png` — **smeared live map vs clean de-drifted map** (best "we
  removed drift" visual).
- `testrun/june7_set7/FINAL_berlin_dedrift.png` — **inner box + outer wall + optimal racing line** (the
  deliverable).
- `testrun/june7_set6/set6_envelope.png` — how the driven laps become a clean centerline (blue inner / yellow
  outer / red center).

**Three notebook/CLI visualizers** (in `scripts/visualizers/`, full code in the Appendix):

| Tool | What it shows | Pros | Cons |
|---|---|---|---|
| `scan_yaml_visualizer.py` | one `/scan` as polar + Cartesian | see exactly what the slice returns; filename encodes height/range/location | static (one scan); needs you to dump YAMLs |
| `maplogger_chronologic_visualizer.py` | steps through a lap CSV chunk-by-chunk (slider/play) | **watch** the walls build over time; catch where it goes wrong | Jupyter only; needs ipywidgets |
| `maplogger_stats_visualizer.py` | odom-drift stats + 4 plots (incl. trajectory-by-time) | one number for drift (start↔end error); the slide chart | needs a closed-loop CSV to be meaningful |

**`build_raceline_from_bag.py`** (laptop) — the one-command pipeline. **Pros:** deterministic, measured widths,
de-drift-friendly. **Cons:** widths only as good as the map (under-observed box → partly interpolated).
